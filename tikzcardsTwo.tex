...
\newcommand{\cardprice}[1]{
    ...
    %\node at (\stripwidth*1.8,\cardheight-.8*\stripwidth) {\InvMoney};
		\node at (\stripwidth*1.7,\cardheight-1.1*\stripwidth) {\color{currentcolor} \textbf{\LARGE #1}};
}

\newcommand{\cardhp}[1]{
		\node[left] at (\cardwidth-\stripwidth*1,\cardheight-3*\stripwidth) {\textbf{\LARGE #1}\Health};
}